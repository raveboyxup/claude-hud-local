...
一个 Claude Code 插件，实时显示多行状态栏——上下文使用率、活跃工具、运行中的 Agent 和待办进度。始终在你的输入下方可见。
...
| **待办进度** | 实时跟踪任务完成情况 |
| **本地模型支持** | 通过 OpenAI 兼容 API 在使用本地模型时也能显示上下文信息 |
...
### 本地模型模式
使用本地模型 API 且没有 Claude Code 订阅使用率数据时：
```
[llama3.1] │ my-project git:(main*)
上下文 █████░░░░░ 62% │ 128k (本地)
```
- 使用本地模式时显示 `(本地)` 标识
- 上下文百分比基于本地 API 返回的模型上下文窗口大小计算

### 可选行（通过 `/claude-hud:configure` 启用）
```
◐ Edit: auth.ts | ✓ Read x3 | ✓ Grep x2        ← 工具活动
...
- 约每 300ms 更新一次
- **本地模型回退** — 当 Claude Code 未提供 `context_window_size` 时，使用本地 API 的模型上下文窗口

---

## 本地模型配置

对于本地模型设置（Ollama、LMStudio、vLLM、自定义代理），Claude HUD 可以查询你的本地 API 获取模型上下文窗口信息。

### 工作原理

1. Claude HUD 请求 `http://127.0.0.1:8086/v1/models`（默认端点）
2. 从响应中读取第一个模型的上下文窗口大小
3. 使用该大小作为上下文百分比计算的基数
4. 在上下文显示中标注 `(本地)` 标识
...
在 `~/.claude/plugins/claude-hud/config.json` 中设置本地 API 端点：
...
如果端点不可达或返回无效数据，Claude HUD 会回退到正常行为（无原生数据时隐藏上下文百分比）。
...
| `display.localApiUrl` | string | `"http://127.0.0.1:8086"` | 本地模型 API 端点，用于获取上下文窗口信息 |
...
**本地模型模式：** `[llama3.1] │ my-project git:(main*)`
```
上下文 █████░░░░░ 62% │ 128k (本地)
```
...
**本地模型不显示？**
- 验证你的本地 API 正在运行且可通过 `display.localApiUrl` 访问
- 检查 API 返回了 OpenAI 兼容的 `/v1/models` 响应
- 确保列表中的第一个模型包含上下文窗口大小（`n_ctx_train` 或 `n_ctx`）
- 仅在获取到有效的上下文窗口大小时才会显示上下文百分比
